...
public class SourceCompletionProvider extends DefaultCompletionProvider
{
	 ...
	 */
	private JythonCompletionProvider jythonProvider;
	...
	private void addCompletionsForPredefinedMembers(Set<Completion > set,
		...
		if (superClass!=null) {
			addCompletionsForPredefinedMembers(set, cu, superClass, pkg);
		}
	}
	...
	protected List<Completion> getCompletionsImpl(JTextComponent comp) {
		...
		try {
			...
			CompilationUnit cu = jythonProvider.getCompilationUnit();
			if (cu==null) {
		...
		if (prefix.equalsIgnoreCase( "cip" ) && !matched) {
			...
			if (matches!=null) {
				for (int i=0; i<matches.size(); i++) {
					...
					addCompletionsForPredefinedMembers(retVal, cu, cf, pkg);
				}
				matched = true;
			}
		}
	...
	private void loadImportCompletions(Set<Completion> set, String text,
		...
		loadCompletionsForImport(set, CIP_PACKAGE, pkgName);
	 ...
	 *
	 * @param jythonProvider The parent completion provider.
	 */
	void setJythonProvider(JythonCompletionProvider jythonProvider) {
		this.jythonProvider = jythonProvider;
	}
}
