 ...
 * amongst multiple {@link JythonCompletionProvider} instances.
 *
...
*/
		...
		if (text.indexOf('.')>-1) {
			String[] pkgNames = org.fife.rsta.ac.java.Util.splitOnChar(text, '.');
	...
	public ClassFile getClassEntry(String className) {
		...
		String[] items = org.fife.rsta.ac.java.Util.splitOnChar(className, '.');
	...
	public List<ClassFile> getClassesWithUnqualifiedName(String name,
		...
		// Check the CIP package
		qualified = "nvisio.cip." + name;
		entry = getClassEntry(qualified);
		if (entry!=null) {
			if (result==null) {
				result = new ArrayList<ClassFile>(1); // Usually small
			}
